...
async fn list_users(pool: web::Data<Pool>) -> HttpResponse {
    ...
    return match user::User::all(&**client).await {
        Ok(list) => {
            let db_res_time = Instant::now() - before;
            let serv_timing = format!("db;dur={}", db_res_time.as_millis());
            HttpResponse::Ok().append_header(("Server-Timing", serv_timing, )).json(list)
        },
        Err(err) => {
